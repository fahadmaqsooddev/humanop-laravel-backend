...
//    public $selected_embedding = "SELECT AN EMBEDDING";
    ...
    public function createEmbedding(){
        try {
            ...
            // Send the request
            $aiReply = $this->sendRequestFromGuzzle('POST', 'http://54.227.7.149:8000/upload_embedding', [
                'multipart' => $multipart
            ]);
...
//            $embedding = HaiChatActiveEmbedding::getChatActiveEmbedding($this->bot_name);
            ...
            $aiReply = $this->sendSearchEmbeddingRequestFromGuzzle('post', 'http://54.227.7.149:8000/search_embeddings', ['query' => $this->query, 'file_name' => $embedding['file_name'], 'total_chunks' => $chatSetting['chunk'], 'loc' => $subFolder]);
...
//    }
    ...
    public function changeEmbeddingStatus($request_id){
        ...
        if($this->request_id && $this->bot_name){
            ...
            $activeEmbedding = HaiChatActiveEmbedding::singleActiveEmbedding($this->request_id, $this->bot_name);
            if($activeEmbedding){
                $aiReply = $this->sendEmbeddingRequestFromGuzzle('post', 'http://54.227.7.149:8000/check-embeddings', ['folder_n' => $this->request_id, 'loc' => $subFolder]);
                 if($aiReply){
                     if($aiReply['exists']){
...
//        $this->activeGroups = EmbeddingGroup::activeGroups();
        ...
        return view('livewire.admin.new-hai-chat.setting.embedding');
    }
}
