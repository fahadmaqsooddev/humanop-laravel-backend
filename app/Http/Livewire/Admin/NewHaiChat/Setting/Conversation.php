        ...
        try {
            ...
            if (!$this->is_restricted_word){
                ...
                if ($this->user_id){
                    ...
                    $user_grid = Assessment::getLatestAssessment($this->user_id);
...
//                    $interval_life = User::userIntervalOfLife($user['date_of_birth']);
                    ...
                    $optimizationPlan = $user_grid ? ActionPlan::getUserActionPlan($this->user_id) : null;

                    $coreState = $user_grid ? Assessment::getCoreState($user_grid, $user['date_of_birth']) : null;

                    $userTrait = Assessment::UserTraits($this->user_id);

                    $userDailyTip = UserDailyTip::where('user_id', $this->user_id)->with('dailyTip')->latest()->first();
    ...
    public function render()
    {
        ...
        return view('livewire.admin.new-hai-chat.setting.conversation', ['conversation' => $this->conversations]);
