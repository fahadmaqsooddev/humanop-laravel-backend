...
class SoundTrackController extends Controller
{
    ...
    private function trackLists($type = 'audio')
    {
        ...
        // 🔹 Grid Names
        $allGridNames = $grids->flatten()->pluck('grid_name')
            ->merge($shopGrids->flatten()->pluck('grid_name'))
            ->unique()
            ->filter();
        ...
        foreach ($allLibraries as $item) {
            ...
            $gridPublicName = $gridsForItem->map(fn($g) =>
                $codeDetails->get($g['grid_name'])->public_name ?? null
            ...
            if ($type === 'audio') {
                ...
                $audioURL = Helpers::extractFilePath($audioUrl);
            } else {
                $videoUrl = null;

                if (!empty($item->source_id)) {
                    $videoUrl = Helpers::getGumletPlaybackUrl($item->source_id);
                }
                ...
                $videoURL = Helpers::extractFilePath($newVideoURL);
            }

            // Skip photo-only resources (no audio/video)
            ...
            $data = [
                ...
                'content' => $item->content,
                'relevance' => $item->relevance,
                'photo_url' => Helpers::extractFilePath($photoUrl),
                'thumbnail_url' => Helpers::extractFilePath($thumbUrl),
        ...
        foreach ($allShopResources as $resource) {
            ...
            $gridPublicName = $gridsForResource->map(fn($g) =>
                $codeDetails->get($g['grid_name'])->public_name ?? null
            )->filter()->values()->toArray();
            ...
            if ($type === 'audio') {
                ...
                $videoUrl = Helpers::getVideoUrl(
                    $resource->upload_id,
                    $uploads,
                    null,
                    $resource->video_embed_link
                );

                $videoURL = Helpers::extractFilePath($videoUrl);
            }

            // ❌ SKIP if no media
            if (!empty($resource->document_url) || !empty($resource->image_url)) {
                continue;
            }
