                ...
                <tr class="table-text-color">
                    ...
                    <td>
                        <button class="btn btn-sm text-white" data-bs-toggle="modal"
                                wire:click="updateOptimizationModal({{ $plan['id']}},`{{$plan['condition']}}`,`{{$plan['priority']}}`,`{{$plan['fourteen_days_plan']}}`)"
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="input-group w-100" wire:ignore style="color: #1b3a62">
                                        <textarea class="form-control input-form-style" id="editor" rows="10" cols="10"
                                                  name="fourteen_days_plan"
                                                  wire:model="fourteen_days_plan"></textarea>
                                    </div>
                                </div>
                            </div>
        ...
        if (editorElement && !editorElement.classList.contains('ck-editor')) { // Check if not already initialized
            ClassicEditor
                ...
                .then(editor => {
                    ...
                    @this.set('fourteen_days_plan', editor.getData())
                        ;
                    })
                    Livewire.on('fourteenDaysPlanUpdated', fourteen_days_plan => {
                        editor.setData(fourteen_days_plan); // Set new content into CKEditor
                    });
