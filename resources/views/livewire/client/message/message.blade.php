<div class="body-container d-flex flex-column flex-md-row align-items-center align-items-md-stretch m p-2 pb-5">
        ...
        <div class="side-bar flex-grow-1 d1 w-100 w-md-30 border border-dark rounded-3 shadow-sm p-0 bg-white " id="message-bar">
            ...
            <div class="tab-content">
                <div class="tab-pane" id="tabs-2" role="tabpanel">
                    @foreach($connections as $connection)
                        <div class="messages-list d-flex align-items-center mx-4 my-3 gap-3" onclick="scrollToSection('messenger-container1')">
                            <div class="chathead flex-shrink-0"
                                style="background-color: #e4dede; width: 60px; height: 60px; border-radius: 50%;">
                                <img src="{{$connection['friend']['user_picture_url'] ?? null}}"
                    ...
                    @foreach($chats as $chat)
                        <div class="messages-list d-flex align-items-center mx-4 my-3 gap-3" onclick="scrollToSection('messenger-container1')">
                            @if($chat['user_data'])
                                <div class="chathead flex-shrink-0"
                                    style="background-color: #e4dede; width: 60px; height: 60px; border-radius: 50%;">
                                    <img src="{{$chat['user_data']['user_picture_url'] ?? null}}"
                                        class="img-fluid rounded-circle">
                                </div>
                                <div class="chatlist cursor-pointer flex-grow-1 bg-dark-blue text-white rounded-2 p-2"
    ...
    @if($chat_user)
        <div class="messenger-container show-messenger pb-2   flex-grow-1 w-100  w-md-70 border border-dark rounded-3 shadow-sm bg-light d-flex flex-column "
            ...
            <div
                ...
                <img src="{{$chat_user['user_picture_url'] ?? null}}" alt="user picture" class="rounded-circle"
            ...
            <form wire:submit.prevent="sendMessage" class="messenger-input-form d-flex h-auto align-items-center   justify-content-center border-top">
                <input type="text" wire:model="message" id="user-input" class="form-control p-1 border-0 "
                    placeholder="Type your message...">
                <button type="submit" class="btn btn-danger d-flex align-items-center m-0"><i class="ni ni-send"></i></button>
            </form>
        </div>
    @else
