<div>
    ...
    <div class="row mt-2">
        ...
        <div class="col-12 col-lg-8">
            <button class="btn bg-gradient-primary" wire:click="$emit('createPostModal')"> <i
                    class="fas fa-plus pe-2"></i> Create Post</button>

            {{-- hidden--}}
            <button id="create_post" class="btn bg-gradient-primary" data-bs-toggle="modal"
                data-bs-target="#create-post-modal" hidden>
                Create Post
            </button>
                ...
                <div class="card main-post-section px-0">
                    <div class="card-header d-flex align-items-center border-bottom py-3">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="mx-3">
                                <a href="javascript:;"
                                    class="text-dark font-weight-600 fs-10px ">{{$post['user'] ? $post['user']['first_name'] . ' ' . $post['user']['last_name'] : "John Snow" }}</a>
                                <small class="d-block text-muted fs-7px">{{$post['created_at']}}</small>
                            </div>
                        </div>
                        ...
                        <div class="text-end ms-auto ">
                            @if($logged_in_user->id === $post->user_id)
                                ...
                                <button wire:click="editPost({{$post->id}})" type="button"
                                    class="btn bg-gradient-primary mb-0 text-white ">
                                    <i class="fa fa-pencil"></i>

                                    {{-- hidden--}}
                                    <button id="edit_post" class="btn bg-gradient-primary" data-bs-toggle="modal"
                                        data-bs-target="#edit-post-modal" hidden>
                                        Edit Post
                                    </button>
                                </button>
                            ...
                            @else
                                ...
                                <button wire:click="followUser({{$post->user_id}})" type="button"
                                    class="btn btn-sm {{ ($post->user->is_follow ?? false) ? 'bg-secondary' : 'bg-gradient-primary' }} mb-0"
                                    style="color: white;">
                                    {{$post->user && $post->user->is_follow ? 'Following' : 'Follow'}}
                                </button>
                    ...
                    <div class="card-body px-0">
                        ...
                        @if($post['photo_url'])
                            <div class="mx-auto">
                                <img alt="Image placeholder" src="{{$post['photo_url']['url']}}"
                            ...
                            <div class="card">
                                <div class="card-header d-flex align-items-center border-bottom py-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="mx-3">
                                            <a href="javascript:;"
                                                class="text-dark font-weight-600 fs-10px ">{{$post['sharedPost']['user'] ? $post['sharedPost']['user']['first_name'] . ' ' . $post['sharedPost']['user']['last_name'] : "John Snow" }}</a>
                                            <small class="d-block text-muted">{{$post['sharedPost']['created_at']}}</small>
                                        </div>
                                    </div>
                                    <div class="text-end ms-auto fs-7px">
                                        @if($logged_in_user->id === $post['sharedPost']['user_id'])

                                        @else

                                            <button wire:click="followUser({{$post['sharedPost']->user_id}})" type="button"
                                                class="btn btn-sm  {{($post['sharedPost']->user->is_follow ?? null) ? "bg-secondary" : "bg-gradient-primary"}} mb-0"
                                                style="color: white">
                                ...
                                <div class="card-body">
                                    ...
                                    @if($post['sharedPost']['photo_url'] ?? null)
                                        <div class="mx-auto">
                                            <img alt="Image placeholder" src="{{$post['sharedPost']['photo_url']['url'] ?? null}}"
                                                class="img-fluid border-radius-lg shadow-lg w-100">
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="col-sm-6">
                                <div class="d-flex text-white">
                                    <a wire:click='postLike({{$post->id}})' class="d-flex align-items-center text-white">
                                        <i
                                            class="ni ni-like-2 me-1 cursor-pointer {{ $post['is_post_liked'] ? "text-orange" : "text-white" }}"></i>
                                        <span class="text-sm me-3">{{$post['post_likes_count']}}</span>
                                    </a>
                                    <div class="d-flex align-items-center">
                                        <i wire:click="showComments({{$post->id}})"
                                            class="ni ni-chat-round me-1 cursor-pointer"></i>
                                        <span class="text-sm me-3">{{$post['post_comments_count']}}</span>

                                        {{-- Hidden--}}
                                        <button id="post_comments" class="btn bg-gradient-primary" data-bs-toggle="modal"
                                            data-bs-target="#post-comments-modal" hidden>
                                            Post Comments
                                        </button>
                                    </div>
                                    ...
                                    {{-- Hidden--}}
                                    <button id="share_post" class="btn bg-gradient-primary" data-bs-toggle="modal"
                                        data-bs-target="#share-post-modal" hidden>
                                        Share Post
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="mb-1">
                            <div class="d-flex">
                                ...
                                <div class="flex-grow-1 my-auto">
                                    <div class="">
                                        <div wire:click="showComments({{$post->id}})"
                                            class="form-control text-secondary cursor-pointer">Comment as
                                            {{$logged_in_user->first_name}}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        {{-- <button type="button" wire:click="$emit('createPostModal')"
                            class="close btn btn-close text-danger" data-dismiss="modal" aria-label="Close"><span
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        {{-- <button type="button" wire:click="$emit('postEditModal')"
                            class="close btn btn-close text-danger" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">X</span></button>--}}
                    </div>
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        {{-- <button type="button" wire:click="$emit('postShareModal')"
                            class="close btn btn-close text-danger" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">X</span></button>--}}
                    </div>
    ...
    <div>
        <div class="modal fade" id="post-comments-modal" tabindex="-1" role="dialog" aria-hidden="true"
            wire:ignore.self>
            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        {{-- <button type="button" wire:click="$emit('postCommentsModal')"
                            class="close btn btn-close text-danger" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">X</span></button>--}}
                    </div>
                                ...
                                <div class="d-flex p-1">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="text-black-50"
                                            style="padding: 5px; border-radius: 9px; width: max-content; background-color: #ebe6e6;max-width: 423px; word-wrap: break-word;">
                                            <span class="h5 mt-0" style="font-size: 12px; font-weight: 600;">
                                                ...
                                                <span>
                                                    @if($comment->user_id === $logged_in_user->id)
                                                        &nbsp;<a wire:confirm="Are you sure you want to delete comment ?"
                                        ...
                                        <div class="d-flex">
                                            <a wire:click='commentLikes({{$comment->id}})'>
                                                <i
                                                    class="ni ni-like-2 me-1 cursor-pointer {{$comment['is_liked_comment'] ? "text-orange" : "text-secondary"}}"></i>
                                            </a>
                                ...
                                <div class="flex-grow-1 my-auto">
                                    <form>
                                        <div class="input-group">
                                            <textarea wire:model="post_comment" class="form-control border-end"
                                                aria-label="With textarea" rows="1"
                                                placeholder="Comment as {{$logged_in_user->first_name}}"></textarea>
                                            <div class="input-group-prepend">
                                                <a type="button" wire:click="createPostComment({{$post_id}})"
                                                    class="input-group text-orange p-3"
...
@push('javascript')
    ...
    <script>
        ...
        window.livewire.on('hideSuccessAlert', () => {
            setTimeout(function () {
