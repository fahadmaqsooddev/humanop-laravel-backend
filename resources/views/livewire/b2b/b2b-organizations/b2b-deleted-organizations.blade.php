        ...
        <table class="table table-flush">
            ...
            @foreach($users as $key => $user)
                <tr class="text-color-blue">
                    ...
                    <td>
                        <button onclick="confirmBoxForRestoreB2BUser({{$user->id}})" class="btn updateBtn" title="restore">
                            <i class="fa-solid fa-rotate-right"></i>
                        </button>

                        <button class="btn btn-danger" title="delete permanently"
                        onclick="confirmBoxForPermanentDeleteB2B({{$user->id}})">
...
<script>
    ...
    function confirmBoxForPermanentDeleteB2B(user_id){
        ...
        swalWithBootstrapButtons.fire({
            ...
            if(result.isConfirmed){
                window.livewire.emit('deleteB2BAdmin', [user_id])
            }
        })
    }
    ...
    function confirmBoxForRestoreB2BUser(user_id){
        ...
        swalWithBootstrapButtons.fire({
            title: '<span style="color: white;">Are you sure?</span>',
            html: "<span style='color: white;'>Want to restore B2BAdmin user!</span>",
            showCancelButton: true,
            confirmButtonText: 'Restore',
        }).then((result) => {
            if(result.isConfirmed){
                window.livewire.emit('restoreB2BAdmin', [user_id])
            }
        })
    }
