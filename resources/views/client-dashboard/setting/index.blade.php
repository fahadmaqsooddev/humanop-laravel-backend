    ...
    <div class="container-fluid my-3 py-3">
        <div class="row mb-5">
            <div class="col-lg-3 ">
                <div class="card position-sticky top-1 background-none">
                    <ul class="nav flex-column border-radius-lg p-3 rainbow-border-user-nav-btn" >
                        ...
                        <li class="nav-item pt-2">
                            <a class="nav-link text-body setting-options" data-scroll="" href="#two-way-auth">
                                <div class="icon me-2">
            ...
            <!-- Card Basic Info -->
                                            ...
                                            <div class="d-flex">
                                                <div class="d-flex">
                                                    ...
                                                    <div>
                                                        ...
                                                        <h6 class="text-white mb-0">{{$user['pm_exp_month'] ? $user['pm_exp_month'] : '**'}}
                                                            /{{$user['pm_exp_year'] ? $user['pm_exp_year'] : '****'}}</h6>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <table class="table table-flush" id="datatable-search">
                                        ...
                                        @foreach($payment_history as $history)
                                            <tr class="text-color-dark">
                                                ...
                                                <td class="text-sm font-weight-normal text-center">{{$history['discount_price']}}</td>
                                                <td class="text-sm font-weight-normal text-center">{{$history['total_price']}}</td>
                                                <td class="text-sm font-weight-normal text-center">{{\Carbon\Carbon::parse($history['created_at'])->format('M, d, Y h:i A')}} (GMT)</td>
                                            </tr>
                                        @endforeach
                                        </tbody>
                                    </table>
                ...
                @livewire('client.setting.two-way-auth')
                <!-- Card Delete Account -->
            ...
            if (cropper) {
                ...
                canvas.toBlob(function (blob) {
                    ...
                    reader.onloadend = function () {
                        ...
                        // Example of appending the file to a FormData object
                        const formData = new FormData();
                        formData.append('image', file);

                        $.ajax({
                            type: "POST",
                            url: '{{ route("user_profile_image") }}',
                            headers: {
                                'X-CSRF-TOKEN': "{{csrf_token()}}"
                            },
                            data: formData,
