...
@if (\Request::is('pages-rtl'))
    ...
    @else
        ...
        <body
            class="{{\App\Helpers\Helpers::getWebUser() ? 'body-background'  : 'left-nav-blue-light-color'}}   g-sidenav-show / {{ (\Request::is('pages-rtl') ? 'rtl' : (Request::is('dashboard-virtual-default')||Request::is('dashboard-virtual-info') ? 'virtual-reality' : (Request::is('authentication-error404')||Request::is('authentication-error500') ? 'error-page' : ''))) }} ">
        @auth
            @yield('auth')
        @endauth
        @guest
