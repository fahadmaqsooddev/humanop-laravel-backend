    ...
    <div class="row">
        <div class="col-lg-9 col-12 mx-auto">
            <div class="card card-body mt-4">
                ...
                <img src="{{ URL::asset('assets/img/items/'.$item->file) }}" class="avatar-xxl" id="imgDisplay"
                     alt="Profile Photo">
              </span>
                        </div>
                    </div>
                  ...
                  <textarea class="editor-content" id="editor" name="description">
                        ...
                        <select class="form-control" name="category_id" id="choices-category">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id}}"
                                        {{ $category->id == $item->category->first()->id ? 'selected' : '' }} class="text-capitalize">{{ $category->name }}</option>
                            @endforeach
                        </select>
                        @error('category_id')
                        <p class="text-danger text-xs mt-2 mb-2">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select class="form-control" name="tag_id[]" id="choices-tag" multiple>
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id}}"
                                        {{ in_array($tag->id, old('tags', $item->tags->pluck('id')->toArray()) ?? []) ? 'selected' : '' }} class="text-capitalize">{{ $tag->name }}</option>
                            @endforeach
                        </select>
                        @error('tag_id')
                        <p class="text-danger text-xs mt-2 mb-2">{{ $message }}</p>
                        ...
                        <div class=" d-flex flex-column">
                            <div class="form-check">
                                <input type="radio" class="form-check-input"
                                       {{  ($item->status == 'Published' ? ' checked' : '') }} name="status"
                                       id="published" value="Published">
                                <label class="custom-control-label" for="published">Published</label>
                            </div>
                            <div class="form-check">
                                <input type="radio" class="form-check-input"
                                       {{  ($item->status == 'Draft' ? ' checked' : '') }} name="status" id="draft"
                                       value="Draft">
                                <label class="custom-control-label" for="draft">Draft</label>
                            </div>
                            <div class="form-check">
                                <input type="radio" class="form-check-input"
                                       {{  ($item->status == 'Active' ? ' checked' : '') }} name="status" id="Active"
                                       value="Active">
                    ...
                    <div class="form-check form-switch mt-4">
                        <input class="form-check-input" type="checkbox" name="show_homepage" id="flexSwitchCheckDefault"
                               onclick="notify(this)"
                               {{  ($item->show_homepage == 'on' ? ' checked' : '') }} data-type="warning"
                        ...
                        <div class=" d-flex flex-column">
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input"
                                       {{ in_array('First', json_decode($item->options) ?? []) ? 'checked' : ''}}  name="option[]"
                            ...
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input"
                                       {{ in_array('Second', json_decode($item->options) ?? []) ? 'checked' : ''}} name="option[]"
                                       id="second" value="Second">
                                <label class="custom-control-label" for="second">Second</label>
                            </div>
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input"
                                       {{ in_array('Third', json_decode($item->options) ?? []) ? 'checked' : ''}} name="option[]"
                    ...
                    <div>
                        ...
                        <div>
                            <input class="form-control datetimepicker flatpickr-input" value="{{ $item->date }}"
                                   name="date" type="text" placeholder="Please select date" data-input=""
                                   onfocus="focused(this)" onfocusout="defocused(this)">
                        </div>
                    </div>
                    <div class="d-flex justify-content-end mt-4">
                        <a href="{{ url('laravel-items-management') }}" type="button" name="button"
